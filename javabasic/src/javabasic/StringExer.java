...
public class StringExer {
	...
	public static void main(String[] args) {
		...
		//문자열의 index/길이(length)
		//index = 0 ~ length-1
		String str3 = "Hello Java"; //index 0~9, lenghth 10
		...
		// 변수에 값을 담아두고 변수를 사용하는 것이 반복적으로 메소드를 호출하지 않는 좋은 코드
	
		int leng = str3.length();
//		for ( int i=0; i<str3.length(); i++) {    //성능차이(안좋은 예)
...
//		System.out.println(str4);
		System.out.println("Hi There");  //한 번만 만들거면 바로 프린트로 출력
...
//		for : 구문을 사용해서 배열의 요소(element)들을 출력
		...
		int i ;
		//i는 main 메소드 내에서 선언된 지역변수이므로 main메서드 밖에서는 사용 불가.
		//i는 main 메소드의 지역변수이므로 초기화 후에만 사용가능
//		System.out.println("선언 후 :"+i);
		...
		String exerStr = " Time is a kind friend, he will make up old 010/1234/5678 ";
		...
		System.out.println(exerStr.indexOf("0"));
		System.out.println(exerStr.replaceAll("/","-").substring(exerStr.indexOf("0"), exerStr.indexOf("0")+13));
		...
		System.out.println("-----------------------------------------");
		//실습 6) i 문자의 인덱스들을 출력
		System.out.println(exerStr.indexOf("i")); // 첫번째 i의 인덱스만 출력
		int startIdx = 0;
		for (int j=0; j<5; j++) {
			startIdx = exerStr.indexOf("i",startIdx+1);
			System.out.println(startIdx);
		}
		//실습 7) 전체 문자열을 소문자로 한 번, 대문자로 한번 변경해서 출력
